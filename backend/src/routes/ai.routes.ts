...
const CATEGORIES = ["Water", "Electricity", "Road", "Waste", "Health"] as const;
const PRIORITIES = ["Low", "Medium", "High", "Critical"] as const;

function heuristicTriage(description: string) {
  const text = description.toLowerCase();
  let suggestedCategory: string = "Road";
  ...
  const suggestedPriority =
    text.includes("urgent") || text.includes("danger") || text.includes("emergency")
      ? "Critical"
      : "Medium";
...
Citizen question: ${message}`;
    ...
    res.json({
      reply:
        "I'm having trouble reaching the AI service right now. You can still file a complaint from the Submit page, or check your complaint history for status updates.",
    });
  }
});
...
router.post("/triage", async (req, res) => {
  ...
  try {
    const prompt = `Classify this civic complaint. Respond with ONLY a JSON object like {"category": "...", "priority": "..."} — category must be exactly one of ${CATEGORIES.join(
      ", "
    )}, priority must be exactly one of ${PRIORITIES.join(", ")}. No other text.
